...
def vit_base_patch16_224(CFG):
    ...
    """
    model = VisionTransformer(img_size=CFG.img_size,
                              patch_size=16,
                              embed_dim=768,
                              depth=12,
                              num_heads=12,
...
def vit_base_patch32_224_in21k(CFG, has_logits: bool = True):
    """
    ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
...
def vit_large_patch16_224_in21k(CFG, has_logits: bool = True):
    """
    ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
...
def vit_large_patch32_224_in21k(CFG, has_logits: bool = True):
    """
...
def vit_huge_patch14_224_in21k(CFG, has_logits: bool = True):
    """
