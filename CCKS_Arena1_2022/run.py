...
# 前两个batch_size=512,ChineseBERT batch_size=200
BERT_PATH = ['uer/chinese_roberta_L-12_H-768','hfl/chinese-roberta-wwm-ext','junnyu/ChineseBERT-large']
...
parser.add_argument("--test_batch", default=40, type=int, help="Test every X updates steps.")

parser.add_argument("--data_dir", default="data", type=str, help="The task data directory.")
parser.add_argument("--model_dir", default="uer/chinese_roberta_L-12_H-768", type=str, help="The directory of pretrained models")
parser.add_argument("--output_dir", default='output/save_dict/', type=str, help="The path of result data and models to be saved.")
# models param
parser.add_argument("--max_length", default=32, type=int, help="the max length of sentence.")
...
def train_entry():
    ...
    for iter in range(5):
        ...
        offset = int(len(train_data_all) * 0.2)
        ...
        # train
        model = Best_Model(config).to(config.device)

        train(config, model, train_iter, dev_iter, iter)

def test_entry(threshold=0.5):
    print("test_entry")
    test_data = load_dataset(config.test_path, config)
    model = Best_Model(config).to(config.device)

    model.load_state_dict(torch.load(config.save_path + f"{model.name}_model.ckpt"))
    model.eval()
    ...
    for i, batches in enumerate(loader):
        ...
        bires = torch.where(pmi > threshold, torch.tensor([1]).cuda(), torch.tensor([0]).cuda())
        # bires = pmi.detach() # 直接输出概率
        for b, t in zip(bires, triple_id):
            predicts.append({"salience": b.item(), "triple_id": t})

    with open(config.save_path + f"{model.name}_{threshold}_result.jsonl", "w") as f:
        for t in predicts:
