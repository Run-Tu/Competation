...
def main():
    ...
    for p in processes:
        p.join()
    # 将共享内存结果转换成dataframe
    result_list = [dict(row) for row in shared_list] # 将 shared_list 转换为通常的包含字典的列表
    valid_df_with_result = pd.DataFrame(result_list)
    valid_df_with_result.to_csv(f"../../data/result/{TODAY}_result.csv", index=False, encoding="UTF-8")
